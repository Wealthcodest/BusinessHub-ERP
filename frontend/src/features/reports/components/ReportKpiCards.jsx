import { CircleDollarSign, CircleCheckBig, ReceiptText, TriangleAlert } from "lucide-react";
...
export default function ReportKpiCards({ metrics, formatMoney }) {
  ...
  return <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">{cards.map(({ label, value, note, icon: Icon, style = "standard" }) => <article key={label} className={`rounded-2xl p-5 shadow-sm ${cardStyles[style]}`}><div className="flex items-start justify-between gap-4"><div><p className={`text-sm ${style === "revenue" ? "text-slate-200" : "text-slate-500"}`}>{label}</p><p className="mt-2 text-2xl font-bold tracking-tight">{value}</p></div><span className={`rounded-xl p-3 ${style === "revenue" ? "bg-white/15" : "bg-teal-50 text-[#18566E]"}`}><Icon className="h-5 w-5" /></span></div><p className={`mt-5 text-sm ${style === "revenue" ? "text-slate-200" : "text-slate-500"}`}>{note}</p></article>)}</div>;
}
